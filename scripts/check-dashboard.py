...
paths = sorted((HERE / "grafana/dashboards").glob("*.json"))
...
for path in paths:
    ...
    all_problems += [f"{path.name}: {x}" for x in problems]
    print(f"  {path.name}: {len(panels)} panels, {len(targets)} link targets")
...
if problems:
    sys.exit(1)
print(f"  ok: {len(paths)} dashboard(s) valid")
